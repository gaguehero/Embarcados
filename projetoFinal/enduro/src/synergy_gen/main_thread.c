...
#undef SYNERGY_NOT_DEFINED	
  .p_callback = g_lcdc_spi_callback,
...
void main_thread_create(void)
{
    ...
    err_g_input_queue = tx_queue_create (&g_input_queue, (CHAR *) "Botan", 1, &queue_memory_g_input_queue,
                                         sizeof(queue_memory_g_input_queue));
    if (TX_SUCCESS != err_g_input_queue)
    {
        tx_startup_err_callback (&g_input_queue, 0);
    }
    UINT err_guixStarted;
    err_guixStarted = tx_mutex_create (&guixStarted, (CHAR *) "Comeco Guix", TX_NO_INHERIT);
    if (TX_SUCCESS != err_guixStarted)
    {
        tx_startup_err_callback (&guixStarted, 0);
    }

    UINT err;
    err = tx_thread_create (&main_thread, (CHAR *) "Main Thread", main_thread_func, (ULONG) NULL, &main_thread_stack,
                            2048, 2, 2, 10, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
