...
void blinky_thread_entry(void)
{
    ...
    while (1)
    {
        ...
        g_sf_comms0.p_api->write(g_sf_comms0.p_ctrl, &enviado, sizeof(enviado),TX_WAIT_FOREVER);

        printf("Pacote %d enviado: %d  \r\n", countt, enviado);

        g_sf_comms0.p_api->read(g_sf_comms0.p_ctrl, &recebido, sizeof(recebido), 30);

        printf("Pacote %d recebido: %d  \r\n", countt, recebido);
